...
class WebsocketClient
{
	...
	// Added function to getData from the socket
	// Adding a timeout
	public function getData() {
		...
		while (1) {
			...
			if ($response != "") {
				$decode = $this->_hybi10Decode($response);
				if ($decode != false) {
					return $decode;
				}
			}
	...
	// Adding sec-websocket-protocol
	public function connect($host, $port, $path, $protocol = "ws", $origin = false)
	{
	...
	private function _hybi10Decode($data)
	{
		...
		/* Only the client is required to send masked frames
		if($isMasked === false)
		{
			$this->close(1002);
		}
